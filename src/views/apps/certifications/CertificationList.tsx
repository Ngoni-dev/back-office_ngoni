...
export default function CertificationList() {
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const res = await certificationService.list(page + 1, perPage, { search: search || undefined, status: status || undefined })
      setItems(res.data ?? [])
  ...
  const review = async (id: number, nextStatus: 'IN_REVIEW' | 'APPROVED' | 'REJECTED' | 'SUSPENDED') => {
    try {
      await certificationService.review(id, {
        status: nextStatus,
  ...
  return (
    <Box sx={{ width: '100%', minWidth: 0 }}>
      <NgoniBreadcrumbs items={[{ label: 'Certifications' }]} />
      <Grid container spacing={6}>
        <Grid size={{ xs: 12 }}>
          <Card>
            <CardHeader title='Demandes certification' subheader='File des demandes et décisions' />
            {!canRead ? (
              <Box className='p-6'>
                <Typography color='text.secondary'>Vous n’avez pas la permission de consulter les certifications.</Typography>
              </Box>
            ) : (
              <>
                        ...
                        <tbody>
                          {items.map(item => (
                            <tr key={item.id}>
                              ...
                              <td>{item.has_certified_badge ? 'Oui' : 'Non'}</td>
                              <td align='right'>
                                <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                                  <button
                                    className='text-primary'
                                    onClick={() => router.push(getLocalizedUrl(`/apps/certifications/${item.id}`, lang as string))}
                                  >
                                    Voir
                                  </button>
                                  {canWrite && (
                                    <>
                                      <button className='text-warning' onClick={() => review(item.id, 'IN_REVIEW')}>Revue</button>
                                      <button className='text-success' onClick={() => review(item.id, 'APPROVED')}>Approuver</button>
                                      <button className='text-error' onClick={() => review(item.id, 'REJECTED')}>Rejeter</button>
                                    </>
                                  )}
                                </Box>
                              </td>
